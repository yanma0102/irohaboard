...
| B-4 | 旧 SHA1 パスワード互換の検証実績 | `05-security.md` §3.2 | **自動テストで実証済み**。`UsersTableTest` に salt 付き/なし SHA1 検証・bcrypt 非再ハッシュ確認、`UsersControllerTest` にログイン→bcrypt 自動アップグレード・誤パスワード拒否の E2E テストを追加（計 7 テスト）。実装は `UserLoginTrait::_login()` / `UsersTable::beforeSave()` のガードで正常動作を確認 | 中（実証済） |

## 7. 対応優先順位（推奨）

> **2026-09-24 進捗**: S-3（PSR-4移行）完了。BUG-1（Infos 追加バグ）、BUG-4（記述式保存失敗）を修正。C-5（uploads/files 統一）完了。D-3 は Bootstrap 3.3.5 確認済み（現状維持方針）。D-5（i18n）、T-4（統合テスト）、T-5（utf8mb4）、T-1（Controller/API テスト全カバー）、T-2（テストスキーマ構築を Migrator に一本化）、D-1（API エラーハンドリングの設計書追記）、D-6（API ルート差分の設計書追記）、B-4（旧 SHA1 パスワード互換の自動テスト実証）も完了。強化試験として Admin/API/モデルのテストギャップを追加（+31 tests）。テストは **380 tests / 1764 assertions** で全パス。B-1 は対応不要。D-2/D-4 は現状維持。残るは B-2（設定値確認）, B-3（本番データ突合）のみ（本番環境が必要）。
...
6. ~~**T-2**（Fixture 相当）~~ ✅対応済（Migrator へ一本化）。~~**D-1**（API エラーハンドリング）・**D-6**（API ルート設計差分）~~ ✅対応済（設計書に実装確定・差分表を追記）。~~**B-4**（旧 SHA1 パスワード互換）~~ ✅対応済（自動テストで実証 — `UsersTableTest`/`UsersControllerTest`）。**D-2・D-4** — 現状維持（機能上問題なし）。**B-2・B-3** — 本番環境での検証（開発環境では実行不可、申し送り）。
...
| データ整合性 (B) | B-1（実害なし確認）, B-4（自動テストで実証済） | B-2, B-3 |
`